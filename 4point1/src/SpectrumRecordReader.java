...
public class SpectrumRecordReader extends RecordReader<Text, RawSpectrum> {
    ...
    public void initialize(InputSplit inputSplit, TaskAttemptContext context) throws IOException {
        ...
        for (int i = 0; i < paths.length; ++i) {
            ...
            //если имя файла в норме - надо считать все строки
    ...
    public boolean nextKeyValue() throws IOException {
        ...
        while (!found && it.hasNext()) {
            ...
            /*
            for (Map.Entry<String, HashMap<Text, float[]>> tmp : hashMap.entrySet()) {
                if (!tmp.getValue().containsKey((Text)entry.getKey())) { contains = false;}
            }
            */
